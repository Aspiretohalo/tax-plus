...
const chapter: any = ref(JSON.parse(sessionStorage.getItem('chapter') || 'null') || '')
const chapter_index: any = ref(route.params.chapter_index)

onMounted(() => {
  // 播放视频需要传入该视频的fileID
  var player = TCPlayer('player-container-id', {
    fileID: chapter.value[chapter_index.value - 1].file_id, // 请传入需要播放的视频fileID 必须
    appID: '1317662942', // 请传入点播账号的子应用appID 必须
    psign: chapter.value[chapter_index.value - 1].psign,
    // 请传入播放器签名psign 必须
